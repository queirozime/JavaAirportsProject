        ...
        while (visitedAirports.size() != airports.size() - 2) {
            ...
            airports.forEach(a -> {
                ...
                if(!a.equals(originAirport)) {
                    ...
                    if(distance.get(a) > distance.get(node.first) + handler.getDistance(a, node.first)) {
                        if(!(a.equals(destinationAirport) && node.first.equals(originAirport))) {

                        parent.put(a,node.first);
                        distance.put(a, Double.valueOf(distance.get(node.first) + handler.getDistance(a, node.first)));
